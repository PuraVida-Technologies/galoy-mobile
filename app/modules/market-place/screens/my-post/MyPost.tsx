...
export const MyPostScreen = () => {
  ...
  const renderTagItem = ({ item }: { item: MarketplaceTag }) => {
    const onSelect = () => {
      const tempTags = [...selectedTag]
      const index = tempTags.findIndex((tag) => tag === item.slug)
  ...
  useEffect(() => {
    const tempFilteredPost = posts.filter((post) => {
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <FlatList
        ...
        ListEmptyComponent={() => (
          <Text style={styles.emptyText}>{t.marketPlace.you_dont_have_any_post()}</Text>
        )}
      />
